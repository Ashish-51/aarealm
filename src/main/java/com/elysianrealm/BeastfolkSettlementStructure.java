...
public class BeastfolkSettlementStructure extends Structure {
    public static final Codec<BeastfolkSettlementStructure> CODEC = RecordCodecBuilder.<BeastfolkSettlementStructure>mapCodec(instance ->
            instance.group(settingsCodec(instance)).apply(instance, BeastfolkSettlementStructure::new)
    ).codec();

    public BeastfolkSettlementStructure(StructureSettings settings) {
        super(settings);
    }
    ...
    private void addPieceAtSurface(StructurePiecesBuilder builder, StructureTemplateManager manager,
        ...
        builder.addPiece(new BeastfolkSettlementPiece(manager, new ResourceLocation(ElysianRealm.MODID, templateName), pos, rotation));
    }

    @Override
    public StructureType<?> type() {
        return ElysianRealm.BEASTFOLK_SETTLEMENT_STRUCTURE.get();
    }
}
