...
public class OrcSettlementStructure extends Structure {
    public static final Codec<OrcSettlementStructure> CODEC = RecordCodecBuilder.<OrcSettlementStructure>mapCodec(instance ->
            instance.group(settingsCodec(instance)).apply(instance, OrcSettlementStructure::new)
    ).codec();

    public OrcSettlementStructure(StructureSettings settings) {
        super(settings);
    }
    ...
    private void addPieceAtSurface(StructurePiecesBuilder builder, StructureTemplateManager manager,
        ...
        builder.addPiece(new OrcSettlementPiece(manager, new ResourceLocation(ElysianRealm.MODID, templateName), pos, rotation));
    }

    @Override
    public StructureType<?> type() {
        return ElysianRealm.ORC_SETTLEMENT_STRUCTURE.get();
    }
}
