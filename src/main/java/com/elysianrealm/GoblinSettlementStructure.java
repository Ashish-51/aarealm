...
public class GoblinSettlementStructure extends Structure {
    public static final Codec<GoblinSettlementStructure> CODEC = RecordCodecBuilder.<GoblinSettlementStructure>mapCodec(instance ->
            instance.group(settingsCodec(instance)).apply(instance, GoblinSettlementStructure::new)
    ).codec();

    public GoblinSettlementStructure(StructureSettings settings) {
        super(settings);
    }
    ...
    private void addPieceAtSurface(StructurePiecesBuilder builder, StructureTemplateManager manager,
        ...
        builder.addPiece(new GoblinSettlementPiece(manager, new ResourceLocation(ElysianRealm.MODID, templateName), pos, rotation));
    }

    @Override
    public StructureType<?> type() {
        return ElysianRealm.GOBLIN_SETTLEMENT_STRUCTURE.get();
    }
}
