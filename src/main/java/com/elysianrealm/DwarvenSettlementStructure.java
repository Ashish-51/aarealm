...
public class DwarvenSettlementStructure extends Structure {
    public static final Codec<DwarvenSettlementStructure> CODEC = RecordCodecBuilder.<DwarvenSettlementStructure>mapCodec(instance ->
            instance.group(settingsCodec(instance)).apply(instance, DwarvenSettlementStructure::new)
    ).codec();

    public DwarvenSettlementStructure(StructureSettings settings) {
        super(settings);
    }
    ...
    public Optional<Structure.GenerationStub> findGenerationPoint(Structure.GenerationContext context) {
        ...
        // 5-point water prevention check
    ...
    private void addPieceAtSurface(StructurePiecesBuilder builder, StructureTemplateManager manager,
        ...
        builder.addPiece(new DwarvenSettlementPiece(manager, new ResourceLocation(ElysianRealm.MODID, templateName), pos, rotation));
    }

    @Override
    public StructureType<?> type() {
        return ElysianRealm.DWARVEN_SETTLEMENT_STRUCTURE.get();
    }
}
