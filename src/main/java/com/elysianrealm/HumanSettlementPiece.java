...
public class HumanSettlementPiece extends TemplateStructurePiece {
    public HumanSettlementPiece(StructureTemplateManager manager, ResourceLocation templateId, BlockPos pos, Rotation rotation) {
        super(ElysianRealm.HUMAN_SETTLEMENT_PIECE.get(), 0, manager, templateId, templateId.toString(), makeSettings(rotation), pos);
    }

    public HumanSettlementPiece(StructurePieceSerializationContext context, CompoundTag tag) {
        super(ElysianRealm.HUMAN_SETTLEMENT_PIECE.get(), tag, context.structureTemplateManager(), (id) -> makeSettings(Rotation.valueOf(tag.getString("Rot"))));
    }
        ...
        if (!level.isClientSide()) {
            ...
            if (box.isInside(spawnPos)) {
                ...
                for (int i = 0; i < count; i++) {
                    ElysianHumanEntity entity = ElysianRealm.ELYSIAN_HUMAN.get().create(level.getLevel());
                    if (entity != null) {
