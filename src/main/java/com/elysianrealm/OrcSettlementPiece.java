...
public class OrcSettlementPiece extends TemplateStructurePiece {
    public OrcSettlementPiece(StructureTemplateManager manager, ResourceLocation templateId, BlockPos pos, Rotation rotation) {
        super(ElysianRealm.ORC_SETTLEMENT_PIECE.get(), 0, manager, templateId, templateId.toString(), makeSettings(rotation), pos);
    }

    public OrcSettlementPiece(StructurePieceSerializationContext context, CompoundTag tag) {
        super(ElysianRealm.ORC_SETTLEMENT_PIECE.get(), tag, context.structureTemplateManager(), (id) -> makeSettings(Rotation.valueOf(tag.getString("Rot"))));
    }
        ...
        if (!level.isClientSide()) {
            ...
            if (box.isInside(spawnPos)) {
                ...
                for (int i = 0; i < count; i++) {
                    ElysianOrcEntity entity = ElysianRealm.ELYSIAN_ORC.get().create(level.getLevel());
                    if (entity != null) {
