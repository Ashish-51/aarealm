...
public class DwarvenSettlementPiece extends TemplateStructurePiece {
    public DwarvenSettlementPiece(StructureTemplateManager manager, ResourceLocation templateId, BlockPos pos, Rotation rotation) {
        super(ElysianRealm.DWARVEN_SETTLEMENT_PIECE.get(), 0, manager, templateId, templateId.toString(), makeSettings(rotation), pos);
    }

    public DwarvenSettlementPiece(StructurePieceSerializationContext context, CompoundTag tag) {
        super(ElysianRealm.DWARVEN_SETTLEMENT_PIECE.get(), tag, context.structureTemplateManager(), (id) -> makeSettings(Rotation.valueOf(tag.getString("Rot"))));
    }
        ...
        if (!level.isClientSide()) {
            ...
            if (box.isInside(spawnPos)) {
                ...
                for (int i = 0; i < count; i++) {
                    ElysianDwarfEntity entity = ElysianRealm.ELYSIAN_DWARF.get().create(level.getLevel());
                    if (entity != null) {
