...
public class HumanSettlementStructure extends Structure {
    public static final Codec<HumanSettlementStructure> CODEC = RecordCodecBuilder.<HumanSettlementStructure>mapCodec(instance ->
            instance.group(settingsCodec(instance)).apply(instance, HumanSettlementStructure::new)
    ).codec();

    public HumanSettlementStructure(StructureSettings settings) {
        super(settings);
    }
    ...
    private void addPieceAtSurface(StructurePiecesBuilder builder, StructureTemplateManager manager,
        ...
        builder.addPiece(new HumanSettlementPiece(manager, new ResourceLocation(ElysianRealm.MODID, templateName), pos, rotation));
    }

    @Override
    public StructureType<?> type() {
        return ElysianRealm.HUMAN_SETTLEMENT_STRUCTURE.get();
    }
}
